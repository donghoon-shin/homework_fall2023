...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        # If using the double-q backup strategy
        if self.target_critic_backup_type == "doubleq":
            # Swap the order of next Q-values
            next_qs = next_qs[[1, 0]]
        # If using the clipped double-q backup strategy
        elif self.target_critic_backup_type == "min":
            # Get the minimum Q-value along dimension 
            # next_qs = torch.min(next_qs, dim = 0)[0] <-- this works too
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # Ensure the shape of the Q-values matches the expected shape
            assert next_qs.shape == (
                self.num_critic_networks,
                batch_size,
            ), next_qs.shape
            
            # If entropy bonus is used and backup entropy is set to True
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                # Add the entropy bonus to the next Q-values
                next_qs += self.temperature * next_action_entropy
            
            # Use the chosen Q-value backup strategy (for double-Q, clip-Q, etc.)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # Copy the observations to match the number of actor samples
            obs_copy = obs.unsqueeze(0).repeat(self.num_actor_samples, 1, 1)
            # Compute Q-values using the critic for the copied observations and the given action
            q_values = self.critic(obs_copy, action)
    ...
    def update(
        ...
        if not isinstance(next_observations, torch.Tensor):
        ...
        # Check for hard target update condition based on the update period
        if self.target_update_period is not None and step % self.target_update_period == 0:
            self.update_target_critic()
        # Check for soft target update condition based on the update rate

...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_obs_ensemble = np.zeros((mb_agent.ensemble_size, ob.shape[0]))
        for i in range(mb_agent.ensemble_size):

            #acs_rep = np.tile(acs, (self.ensemble_size, 1))
            next_obs_ensemble[i,:] = mb_agent.get_dynamics_predictions(i,np.expand_dims(ob, axis=0), np.expand_dims(ac, axis=0))


        next_ob = next_obs_ensemble.mean(axis=0)
...
def run_training_loop(
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                trajs = replay_buffer.sample(config["train_batch_size"])
                loss = mb_agent.update(
                    i,
                    trajs['observations'],
                    trajs['actions'],
                    trajs['next_observations'],)

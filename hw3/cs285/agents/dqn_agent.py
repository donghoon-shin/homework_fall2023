...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        # Now, concatenated_tensor should have shape (2, 5)
        q_values = ptu.to_numpy(self.critic(observation).squeeze(-1))
        
        assert q_values.shape == (1, self.num_actions) 

        action = np.argmax(q_values, axis = -1) if np.random.random() > epsilon\
              else np.array(np.random.choice(self.num_actions))
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                next_action = torch.argmax(next_qa_values_for_action_selection, dim=-1)
            else:
        ...
        assert qa_values.shape == (batch_size, self.num_actions)  
